...
但当前 `SoapClient` 仅在内存中生成 XML；未发起 HTTP SOAP 调用，`mock` 字段也尚未形成真实的客户端切换行为。后续实现必须在不重建上述任务、导入、日志链路的前提下，将该组件替换为“本地 SOAP 客户端 + 本地模拟 SOAP 服务端”。
...
`queryGrowthRecords` 返回本地模拟校内系统当前可提供的生长采集记录。初版仅返回一条成功记录；测试时可通过明确的 mock 场景返回业务失败或 SOAP Fault。
...
后续实现使用以下配置名，真实地址和凭据只能通过环境变量或本地 `.env` 注入：
...
3. 让 `mock` 模式显式选择本地模拟端点；非 mock 模式在未配置真实端点时明确拒绝，不能静默回退为假数据。
4. 接入 `scripts/dev-soap-seed.sql` 中的前置数据，补成功、SOAP Fault、导入匹配失败和重复 `externalNo` 的测试。
...
本方案只证明协议链路和业务导入能力，不代表已与真实校内系统完成联调。真实接入前必须由校方提供 WSDL、环境地址、接口账号/认证方式、字段字典、错误码、调用频率和联调窗口；届时新增适配器实现，不修改本次已冻结的业务导入契约。
