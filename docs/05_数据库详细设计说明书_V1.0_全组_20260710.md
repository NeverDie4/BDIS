...
> 存储引擎：InnoDB  
> 字符集：utf8mb4  
> 技术栈：Next.js + Spring Boot + MyBatis-Plus
> 设计范围：用户权限、中药材资源、地图分布、生长采集、文件资源、图谱知识、AI 图像识别、教学科研、评价申报、业绩认定、SOAP 数据交换、日志审计、首页看板。
> 本次修订：补充 M05、M06、M19 实现所需字段，并同步 `V20260713_004` 的文件关联唯一约束、文件访问失败原因和审计组合索引；同步 M12-M15 的 `V20260714_010` 至 `V20260714_015` 迁移字段、培训素材表及关联约束。
...
| 13 | sys_file_resource | 文件资源表 | M05 文件资源模块 | P0 已实现 |
| 14 | sys_file_business | 文件业务关联表 | M05 文件资源模块 | P0 已实现 |
| 15 | log_login | 登录日志表 | M01 身份认证模块 / M06 操作审计模块 | P0 已实现 |
| 16 | log_operation | 操作日志表 | M06 操作审计模块 | P0 已实现 |
| 17 | log_data_change | 数据变更日志表 | M06 操作审计模块 | P1 |
| 18 | log_file_access | 文件访问日志表 | M05 文件资源模块 / M06 操作审计模块 | P0 已实现 |
| 19 | log_data_sync | 数据同步日志表 | M19 SOAP 数据交换模块 / M06 操作审计模块 | P1 已实现 |
...
| 39A | edu_training_material | 培训素材表 | M15 培训过程模块 | P1 |
| 39B | rel_training_plan_material | 培训计划素材关联表 | M15 培训过程模块 | P1 |
...
| 59 | herb_atlas_feature | 标准图谱特征向量表 | M11 图谱知识模块 | P1 |
| 60 | herb_image_feature | 用户图片特征向量表 | M11 图谱知识模块 | P1 |
| 61 | herb_identification_result | 中药材图片最终识别结果表 | M11 图谱知识模块 | P1 |
| 62 | herb_collection_task | 中药材采集任务表 | M09 生长采集模块 | P1 |
| 63 | herb_batch | 中药材批次档案表 | M09 生长采集模块 / M10 采集审核与溯源模块 | P1 |
| 64 | herb_batch_image | 中药材批次图片关联表 | M09 生长采集模块 / M11 图谱知识模块 | P1 |
| 65 | herb_ai_chat_session | AI 小助手会话表 | AI 小助手模块 | P1 |
| 66 | herb_ai_chat_message | AI 小助手消息表 | AI 小助手模块 | P1 |
| 67 | herb_ai_knowledge_doc | AI 知识库文档表 | AI 小助手模块 / RAG 知识库模块 | P1 |
| 69 | herb_growth_trace_event | 生长记录溯源事件表 | M10 采集审核与溯源模块 | P0 |
| 68 | herb_ai_knowledge_chunk | AI 知识库文档切片表 | AI 小助手模块 / RAG 知识库模块 | P1 |
---
...
erDiagram
    ...
    HERB_GROWTH_RECORD ||--o{ HERB_GROWTH_TRACE_EVENT : traces
    HERB_BATCH ||--o| HERB_GROWTH_RECORD : records
    HERB_IMAGE ||--o{ HERB_IMAGE_RECOGNITION : recognized
    HERB_IMAGE ||--o{ HERB_IMAGE_MATCH : matched
    HERB_IMAGE ||--o{ HERB_IMAGE_FEATURE : extracts
    HERB_IMAGE ||--o{ HERB_IDENTIFICATION_RESULT : identifies
    HERB_ATLAS ||--o{ HERB_IMAGE_MATCH : target
    HERB_ATLAS ||--o{ HERB_ATLAS_FEATURE : extracts
    HERB_SPECIES ||--o{ HERB_ATLAS : owns
    HERB_COLLECTION_TASK ||--o{ HERB_BATCH : produces
    HERB_BATCH ||--o{ HERB_BATCH_IMAGE : contains
    HERB_IMAGE ||--o{ HERB_BATCH_IMAGE : binds
```
...
| storage_path | VARCHAR(500) | 否 | NULL |  | 文件在存储介质中的相对路径 |
| thumbnail_url | VARCHAR(500) | 否 | NULL |  | 缩略图地址 |
| storage_type | VARCHAR(50) | 否 | local | IDX | 存储类型：local、oss、cos |
| access_level | VARCHAR(20) | 是 | private | IDX | 访问级别：private、public |
| content_type | VARCHAR(150) | 否 | NULL |  | 文件 MIME 类型 |
| uploader_id | BIGINT | 否 | NULL | FK/IDX | 上传人 ID |
| uploader_name | VARCHAR(100) | 否 | NULL |  | 上传人名称快照 |
...
**普通索引：** idx_sys_file_resource_file_type(file_type), idx_sys_file_resource_uploader_id(uploader_id), idx_sys_file_resource_uploaded_at(uploaded_at), idx_sys_file_resource_access_level(access_level), idx_sys_file_resource_access_status(access_level, status)
...
| is_public | TINYINT(1) | 是 | 0 | IDX | 该业务关系是否要求文件公开访问；文件级 access_level 由全部关系的发布状态派生 |
| sort_order | INT | 否 | 0 |  | 排序号 |

**主键：** `id`

**唯一约束 / 唯一索引：** uk_sys_file_business_relation(file_id, biz_type, biz_id)

**普通索引：** idx_sys_file_business_file_id(file_id), idx_sys_file_business_biz(biz_type, biz_id), idx_sys_file_business_public(file_id, is_public)
...
**普通索引：** idx_log_login_user_id(user_id), idx_log_login_logged_in_at(logged_in_at), idx_log_login_username_time(username, logged_in_at), idx_log_login_result_time(login_result, logged_in_at)
...
| biz_type | VARCHAR(100) | 否 | NULL | IDX | 业务对象类型 |
| biz_id | BIGINT | 否 | NULL | IDX | 业务对象 ID |
...
**普通索引：** idx_log_operation_operator_id(operator_id), idx_log_operation_module(operation_module), idx_log_operation_time(operation_time), idx_log_operation_biz(biz_type, biz_id), idx_log_operation_type_time(operation_type, operation_time), idx_log_operation_result_time(result_status, operation_time)
...
| result_status | VARCHAR(50) | 否 | NULL | IDX | 访问结果：SUCCESS、FAILED |
| failure_reason | VARCHAR(1000) | 否 | NULL |  | 访问失败原因，仅失败时填写 |
| ip_address | VARCHAR(64) | 否 | NULL |  | IP 地址 |
| user_agent | VARCHAR(500) | 否 | NULL |  | 客户端信息 |
...
**普通索引：** idx_log_file_access_file_id(file_id), idx_log_file_access_operator_id(operator_id), idx_log_file_access_time(operation_time), idx_log_file_access_type_time(access_type, operation_time), idx_log_file_access_result_time(result_status, operation_time)
...
| task_id | BIGINT | 否 | NULL | IDX | SOAP 同步任务 ID |
| exchange_id | BIGINT | 否 | NULL | IDX | SOAP 交换记录 ID |
| biz_type | VARCHAR(100) | 否 | NULL | IDX | 导入业务对象类型 |
| biz_id | BIGINT | 否 | NULL | IDX | 导入业务对象 ID |
| external_no | VARCHAR(100) | 否 | NULL | IDX | 外部系统业务编号 |
| success_count | INT | 是 | 0 |  | 成功记录数 |
| failure_count | INT | 是 | 0 |  | 失败记录数 |
| operator_id | BIGINT | 否 | NULL | FK/IDX | 操作人 ID |
| operator_name | VARCHAR(100) | 否 | NULL |  | 操作人名称快照 |
...
**普通索引：** idx_log_data_sync_sync_type(sync_type), idx_log_data_sync_status(sync_status), idx_log_data_sync_target(target_table, target_id), idx_log_data_sync_task_id(task_id), idx_log_data_sync_exchange_id(exchange_id), idx_log_data_sync_biz(biz_type, biz_id), idx_log_data_sync_external_no(external_no)

**逻辑外键关系：** operator_id -> sys_user.id; task_id -> soap_sync_task.id; exchange_id -> soap_exchange_record.id
...
| cover_image_url | VARCHAR(500) | 否 | NULL |  | 药材封面图片受控访问地址 |
...
- **对应模块**：M09 生长采集模块
- **优先级/状态**：P0
- **表说明**：每个采集批次最多对应一条结构化生长记录；历史数据允许 `batch_id` 为空，新建批次记录必须绑定批次。
- **通用字段组**：`full`，包含主键、启停状态、逻辑删除、创建更新审计、备注和乐观锁字段。

**业务字段设计：**

| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| batch_id | BIGINT | 历史可空/新增必填 | NULL | UK/IDX | 采集批次 ID，一个批次最多一条记录 |
| task_id | BIGINT | 否 | NULL | IDX | 采集任务 ID，趋势查询冗余字段 |
| species_id | BIGINT | 是 | NULL | FK/IDX | 药材品种 ID |
| species_name | VARCHAR(150) | 否 | NULL |  | 药材名称快照 |
| base_id | BIGINT | 否 | NULL | FK/IDX | 基地 ID |
| base_name | VARCHAR(150) | 否 | NULL |  | 基地名称快照 |
...
| plant_height | DECIMAL(10,2) | 否 | NULL |  | 株高 cm |
...
| soil_moisture | DECIMAL(5,2) | 否 | NULL |  | 土壤湿度 % |
| light | DECIMAL(12,2) | 否 | NULL |  | 光照 lx |
| stem_diameter | DECIMAL(10,2) | 否 | NULL |  | 茎粗 mm |
| leaf_color | VARCHAR(64) | 否 | NULL |  | 叶色 |
| flowering_status | VARCHAR(100) | 否 | NULL |  | 开花情况 |
| growth_evaluation | VARCHAR(500) | 否 | NULL |  | 生长评价 |
...
| external_source | VARCHAR(50) | 否 | NULL | UK | 外部来源系统 |
| external_no | VARCHAR(100) | 否 | NULL | UK | 外部业务编号，与来源组成幂等键 |
...
| trace_code | VARCHAR(100) | 否 | NULL | UK | 公开溯源码 |
| trace_qrcode_url | VARCHAR(255) | 否 | NULL |  | 受控二维码资源地址 |
| trace_public_url | VARCHAR(255) | 否 | NULL |  | Web 公开溯源页地址 |
| public_visible | TINYINT | 是 | 0 |  | 是否开启公开溯源 |
| trace_generated_time | DATETIME | 否 | NULL |  | 溯源码生成时间 |
| trace_generated_by | BIGINT | 否 | NULL | FK | 溯源码操作人 ID |
| trace_generated_by_name | VARCHAR(100) | 否 | NULL |  | 溯源码操作人姓名快照 |

**主键：** `id`

**唯一约束 / 唯一索引：** `uk_herb_growth_record_batch_id(batch_id)`、`uk_herb_growth_record_external_identity(external_source, external_no)`、`uk_growth_record_trace_code(trace_code)`

**普通索引：** `idx_herb_growth_record_task_id(task_id)`、`idx_herb_growth_record_base_id(base_id)`、`idx_herb_growth_record_species_id(species_id)`、`idx_herb_growth_record_distribution_id(distribution_id)`、`idx_herb_growth_record_collected_at(collected_at)`、`idx_herb_growth_record_review_status(review_status)`

**逻辑外键关系：** `batch_id -> herb_batch.id`；`task_id -> herb_collection_task.id`；`species_id -> herb_species.id`；`base_id -> herb_base.id`；`distribution_id -> herb_distribution.id`；`collector_id -> sys_user.id`；`region_id -> dict_region.id`
...
| reviewer_name | VARCHAR(100) | 否 | NULL |  | 审核人姓名快照 |
| reviewer_role | VARCHAR(100) | 否 | NULL |  | 审核人角色代码快照 |
...
### 6.24.1 `herb_growth_trace_event`：生长记录溯源事件表

- **对应模块**：M10 采集审核与溯源模块
- **表说明**：记录生长记录创建、修改、提交、审核、归档、生成溯源码/二维码和公开开关事件。

| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| record_id | BIGINT | 是 | NULL | FK/IDX | 生长记录 ID |
| event_type | VARCHAR(50) | 是 | NULL |  | 事件类型 |
| event_title | VARCHAR(100) | 是 | NULL |  | 事件标题 |
| event_content | VARCHAR(500) | 否 | NULL |  | 事件内容 |
| before_status | VARCHAR(50) | 否 | NULL |  | 操作前状态 |
| after_status | VARCHAR(50) | 否 | NULL |  | 操作后状态 |
| operator_id | BIGINT | 否 | NULL | FK | 操作人 ID |
| operator_name | VARCHAR(100) | 否 | NULL |  | 操作人姓名快照 |
| operator_role | VARCHAR(100) | 否 | NULL |  | 操作人角色快照 |
| event_time | DATETIME | 是 | CURRENT_TIMESTAMP | IDX | 事件时间 |
| metadata_json | TEXT | 否 | NULL |  | 内部事件元数据，公开接口不返回 |

**普通索引：** `idx_herb_growth_trace_event_record_id(record_id)`、`idx_herb_growth_trace_event_event_time(event_time)`

**逻辑外键关系：** `record_id -> herb_growth_record.id`；`operator_id -> sys_user.id`
...
**普通索引：** idx_herb_image_species_id(species_id), idx_herb_image_distribution_id(distribution_id), idx_herb_image_growth_record_id(growth_record_id), idx_herb_image_process_status(process_status)
...
**唯一约束 / 唯一索引：** uk_eval_archive_archive_no(archive_no), uk_eval_archive_application_id(application_id)

**普通索引：** idx_eval_archive_owner_id(owner_id)
...
| level_rule | TEXT | 否 | NULL |  | 等级或等级认定规则 |
| standard_version | INT | 是 | 1 | UK | 标准版本号 |
| lifecycle_status | VARCHAR(20) | 是 | draft | IDX | 生命周期：draft、published、disabled |
| effective_from / effective_to | DATETIME | 否 | NULL | IDX | 标准适用起止时间 |
| material_required | TINYINT | 是 | 1 |  | 是否要求佐证材料 |
| min_material_count | INT | 是 | 1 |  | 最少材料数量 |
| published_at / published_by | DATETIME / BIGINT | 否 | NULL |  | 发布审计信息 |
| sort_order | INT | 否 | 0 |  | 排序号 |

**主键：** `id`

**唯一约束 / 唯一索引：** uk_perf_standard_no_version(standard_no, standard_version)
...
| performance_level | VARCHAR(50) | 是 | unspecified | IDX | 业绩等级 |
| occurred_at | DATETIME | 否 | NULL | IDX | 业绩发生时间 |
| standard_id | BIGINT | 否 | NULL | FK/IDX | 认定标准 ID |
| source_type | VARCHAR(50) | 否 | NULL | IDX | 来源类型 |
| source_id | BIGINT | 否 | NULL | IDX | 来源对象 ID |
| source_name_snapshot | VARCHAR(200) | 否 | NULL |  | 来源名称快照 |
| standard_no/version/name/rule_snapshot | VARCHAR/LONGTEXT | 否 | NULL |  | 提交时冻结的标准编号、版本、名称和规则快照 |
...
### 6.54.1 `perf_participant`：业绩参与人表

- **对应模块**：M18 业绩认定模块
- **优先级/状态**：P1 已实现
- **表说明**：保存业绩负责人和参与人，支持负责人、参与人两个统计口径。

| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| performance_id | BIGINT | 是 | NULL | IDX | 业绩 ID |
| user_id | BIGINT | 是 | NULL | IDX | 参与用户 ID |
| participant_role | VARCHAR(50) | 是 | participant |  | 参与角色 |
| sort_order | INT | 是 | 0 |  | 排序号 |
| is_primary | TINYINT | 是 | 0 | IDX | 是否负责人；负责人必须与 perf_record.user_id 一致 |

**逻辑外键关系：** performance_id -> perf_record.id; user_id -> sys_user.id
...
| resource_type | VARCHAR(100) | 否 | NULL |  | 交换资源类型 |
...
| is_mock | TINYINT | 是 | 1 |  | 是否使用模拟 SOAP 响应：1 是，0 否 |
...
| sync_status | VARCHAR(50) | 否 | pending | IDX | 同步状态 |
| error_message | TEXT | 否 | NULL |  | 错误信息 |
| parsed_payload | LONGTEXT | 否 | NULL |  | SOAP 响应解析后的 JSON 数据 |
| called_by | BIGINT | 否 | NULL | FK/IDX | 调用人 ID |
| called_by_name | VARCHAR(100) | 否 | NULL |  | 调用人名称快照 |
...
### 6.59 `herb_atlas_feature`：标准图谱特征向量表

- **对应模块**：M11 图谱知识模块
- **优先级/状态**：P1
- **表说明**：保存标准图谱图片的特征向量和提取状态，用于本地图谱相似度检索。
- **通用字段组**：`full`。
...
| atlas_id | BIGINT | 是 | NULL | FK/IDX | 标准图谱 ID |
| species_id | BIGINT | 否 | NULL | FK/IDX | 药材品种 ID |
| feature_code | VARCHAR(100) | 否 | NULL |  | 特征编码 |
| feature_vector | LONGTEXT | 是 | NULL |  | 特征向量 JSON |
| feature_dimension | INT | 否 | NULL |  | 特征向量维度 |
| feature_model | VARCHAR(100) | 否 | NULL | IDX | 特征提取模型名称 |
| feature_version | VARCHAR(50) | 否 | NULL | IDX | 特征提取模型版本 |
| extract_status | VARCHAR(30) | 是 | pending | IDX | 提取状态：pending、success、failed |
| extract_time | DATETIME | 否 | NULL |  | 特征提取时间 |
...
**普通索引：** idx_herb_atlas_feature_atlas_id(atlas_id), idx_herb_atlas_feature_species_id(species_id), idx_herb_atlas_feature_model(feature_model, feature_version), idx_herb_atlas_feature_status(extract_status, is_deleted)

**逻辑外键关系：** atlas_id -> herb_atlas.id; species_id -> herb_species.id


### 6.60 `herb_image_feature`：用户图片特征向量表

- **对应模块**：M11 图谱知识模块
- **优先级/状态**：P1
- **表说明**：保存用户上传图片的特征向量和提取状态，用于和标准图谱进行相似度匹配。
- **通用字段组**：`full`。
...
**普通索引：** idx_herb_image_feature_image_id(image_id), idx_herb_image_feature_species_id(species_id), idx_herb_image_feature_model(feature_model, feature_version), idx_herb_image_feature_status(extract_status, is_deleted)

**逻辑外键关系：** image_id -> herb_image.id; species_id -> herb_species.id


### 6.61 `herb_identification_result`：中药材图片最终识别结果表

- **对应模块**：M11 图谱知识模块
- **优先级/状态**：P1
- **表说明**：汇总本地图谱匹配、辅助 AI 识别和人工复核，形成图片的最终识别结论。
- **通用字段组**：`full`。
...
| --- | --- | --- | --- | --- | --- |
| image_id | BIGINT | 是 | NULL | FK/IDX | 用户图片 ID |
| best_match_id | BIGINT | 否 | NULL | FK | 最佳本地图谱匹配记录 ID |
| recognition_id | BIGINT | 否 | NULL | FK | 辅助 AI 识别记录 ID |
| final_species_id | BIGINT | 否 | NULL | FK/IDX | 最终识别药材 ID |
| final_species_name | VARCHAR(100) | 否 | NULL |  | 最终识别药材名称 |
| final_confidence | DECIMAL(10,4) | 否 | NULL |  | 最终置信度或相似度 |
| result_source | VARCHAR(50) | 否 | NULL | IDX | 结果来源：local_match、doubao_review、manual_review |
| match_result | VARCHAR(50) | 否 | NULL | IDX | 匹配结果 |
| need_review | TINYINT | 是 | 0 | IDX | 是否需要人工复核 |
| review_status | VARCHAR(50) | 否 | NULL | IDX | 复核状态：pending、confirmed、rejected |
| suggestion | VARCHAR(500) | 否 | NULL |  | 系统建议 |
| review_comment | VARCHAR(500) | 否 | NULL |  | 人工复核意见 |
| reviewer_id | BIGINT | 否 | NULL | FK | 复核人用户 ID |
| reviewer_name | VARCHAR(100) | 否 | NULL |  | 复核人姓名快照 |
| review_time | DATETIME | 否 | NULL |  | 复核时间 |
| raw_summary | LONGTEXT | 否 | NULL |  | 识别流程摘要 JSON |
| identify_time | DATETIME | 否 | NULL |  | 识别完成时间 |

**主键：** `id`

**唯一约束 / 唯一索引：** 无

**普通索引：** idx_herb_identification_result_image_id(image_id), idx_herb_identification_result_species_id(final_species_id), idx_herb_identification_result_source(result_source), idx_herb_identification_result_match(match_result), idx_herb_identification_result_review(need_review, review_status)

**逻辑外键关系：** image_id -> herb_image.id; best_match_id -> herb_image_match.id; recognition_id -> herb_image_recognition.id; final_species_id -> herb_species.id; reviewer_id -> sys_user.id


### 6.62 `herb_collection_task`：中药材采集任务表

- **对应模块**：M09 生长采集模块
- **优先级/状态**：P1
- **表说明**：保存采集任务的目标药材、采集人员、地点和计划时间。
- **通用字段组**：`full`。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| task_code | VARCHAR(64) | 是 | NULL | UK | 采集任务编码 |
| task_name | VARCHAR(255) | 是 | NULL |  | 采集任务名称 |
| species_id | BIGINT | 否 | NULL | FK/IDX | 目标药材品种 ID |
| species_name | VARCHAR(100) | 否 | NULL |  | 目标药材名称快照 |
| base_id | BIGINT | 否 | NULL | FK | 种植基地 ID |
| base_name | VARCHAR(255) | 否 | NULL |  | 基地名称快照 |
| collect_place | VARCHAR(255) | 否 | NULL |  | 采集地点 |
| planned_start_time | DATETIME | 否 | NULL | IDX | 计划开始时间 |
| planned_end_time | DATETIME | 否 | NULL | IDX | 计划结束时间 |
| collector_id | BIGINT | 否 | NULL | FK/IDX | 采集人用户 ID |
| collector_name | VARCHAR(100) | 否 | NULL |  | 采集人姓名快照 |
| task_status | VARCHAR(50) | 是 | draft | IDX | 任务状态：draft、published、in_progress、completed、cancelled |
| description | TEXT | 否 | NULL |  | 任务说明 |

**主键：** `id`

**唯一约束 / 唯一索引：** uk_herb_collection_task_task_code(task_code)

**普通索引：** idx_herb_collection_task_species_id(species_id), idx_herb_collection_task_collector_id(collector_id), idx_herb_collection_task_status(task_status), idx_herb_collection_task_planned_time(planned_start_time, planned_end_time)

**逻辑外键关系：** species_id -> herb_species.id; base_id -> herb_base.id; collector_id -> sys_user.id


### 6.63 `herb_batch`：中药材批次档案表

- **对应模块**：M09 生长采集模块 / M10 采集审核与溯源模块
- **优先级/状态**：P1
- **表说明**：保存采集批次、图片识别汇总、人工确认结果和质量评价。
- **通用字段组**：`full`。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| batch_code | VARCHAR(64) | 是 | NULL | UK | 批次编码 |
| batch_name | VARCHAR(255) | 是 | NULL |  | 批次名称 |
| task_id | BIGINT | 否 | NULL | FK/IDX | 采集任务 ID |
| species_id | BIGINT | 否 | NULL | FK/IDX | 药材品种 ID |
| species_name | VARCHAR(100) | 否 | NULL |  | 药材名称快照 |
| base_id | BIGINT | 否 | NULL | FK | 种植基地 ID |
| base_name | VARCHAR(255) | 否 | NULL |  | 基地名称快照 |
| origin_place | VARCHAR(255) | 否 | NULL |  | 产地或采集地点 |
| collect_start_time | DATETIME | 否 | NULL | IDX | 批次采集开始时间 |
| collect_end_time | DATETIME | 否 | NULL | IDX | 批次采集结束时间 |
| harvest_time | DATETIME | 否 | NULL |  | 采收时间 |
| production_date | DATE | 否 | NULL |  | 生产日期 |
| batch_status | VARCHAR(50) | 是 | draft | IDX | 批次状态 |
| image_count | INT | 是 | 0 |  | 绑定图片数量 |
| identified_count | INT | 是 | 0 |  | 已有识别结果的图片数量 |
| reviewed_count | INT | 是 | 0 |  | 已人工复核图片数量 |
| need_review_count | INT | 是 | 0 |  | 需要人工复核图片数量 |
| final_species_id | BIGINT | 否 | NULL | FK | 最终确认药材 ID |
| final_species_name | VARCHAR(100) | 否 | NULL |  | 最终确认药材名称 |
| avg_similarity | DECIMAL(10,6) | 否 | NULL |  | 平均相似度 |
| quality_level | VARCHAR(50) | 否 | NULL |  | 质量等级 |
| quality_score | DECIMAL(10,4) | 否 | NULL |  | 质量评分 |
| evaluation_summary | TEXT | 否 | NULL |  | 批次评价摘要 |
| trace_code | VARCHAR(100) | 否 | NULL | IDX | 追溯编码 |

**主键：** `id`

**唯一约束 / 唯一索引：** uk_herb_batch_batch_code(batch_code)

**普通索引：** idx_herb_batch_task_id(task_id), idx_herb_batch_species_id(species_id), idx_herb_batch_status(batch_status), idx_herb_batch_trace_code(trace_code), idx_herb_batch_collect_time(collect_start_time, collect_end_time)

**逻辑外键关系：** task_id -> herb_collection_task.id; species_id -> herb_species.id; base_id -> herb_base.id; final_species_id -> herb_species.id


### 6.64 `herb_batch_image`：中药材批次图片关联表

- **对应模块**：M09 生长采集模块 / M11 图谱知识模块
- **优先级/状态**：P1
- **表说明**：保存批次与采集图片的绑定关系、图片角色和最终识别结果引用。
- **通用字段组**：`full`。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| batch_id | BIGINT | 是 | NULL | FK/IDX | 批次 ID |
| image_id | BIGINT | 是 | NULL | FK/IDX | 采集图片 ID |
| identification_result_id | BIGINT | 否 | NULL | FK/IDX | 最终识别结果 ID |
| image_role | VARCHAR(50) | 否 | NULL |  | 图片角色 |
| is_primary | TINYINT | 是 | 0 |  | 是否主图 |
| bind_status | VARCHAR(50) | 是 | bound | IDX | 绑定状态：bound、removed |
| sort_order | INT | 是 | 0 |  | 排序号 |

**主键：** `id`

**唯一约束 / 唯一索引：** 无；当前有效绑定由业务层保证同一批次内不重复。

**普通索引：** idx_herb_batch_image_batch_id(batch_id), idx_herb_batch_image_image_id(image_id), idx_herb_batch_image_result_id(identification_result_id), idx_herb_batch_image_binding(batch_id, image_id, is_deleted)

**逻辑外键关系：** batch_id -> herb_batch.id; image_id -> herb_image.id; identification_result_id -> herb_identification_result.id


### 6.65 `herb_ai_chat_session`：AI 小助手会话表

- **对应模块**：AI 小助手模块
- **优先级/状态**：P1
- **表说明**：保存 AI 小助手普通聊天、RAG 问答、批次解释和图片解释的会话摘要。
- **通用字段组**：轻量业务表。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| session_id | VARCHAR(64) | 是 | NULL | UK/IDX | 前端或后端生成的会话 ID |
| session_title | VARCHAR(100) | 是 | NULL |  | 会话标题 |
| user_id | BIGINT | 否 | NULL | UK/IDX | 会话所属用户 ID |
| source | VARCHAR(20) | 否 | NULL | IDX | 来源：web、mobile、powershell 等 |
| last_message | VARCHAR(500) | 否 | NULL |  | 最后一条消息摘要 |
| last_message_time | DATETIME | 否 | NULL | IDX | 最后消息时间 |
| status | VARCHAR(20) | 是 | normal |  | 状态：normal、deleted |
| create_time | DATETIME | 是 | CURRENT_TIMESTAMP |  | 创建时间 |
| update_time | DATETIME | 是 | CURRENT_TIMESTAMP |  | 更新时间 |
| is_deleted | TINYINT | 是 | 0 | IDX | 逻辑删除：0 未删除，1 已删除 |

**主键：** `id`

**唯一约束 / 唯一索引：** uk_ai_chat_session_user_id(user_id, session_id)

**普通索引：** idx_ai_chat_session_user_time(user_id, last_message_time), idx_ai_chat_session_source(source)

**逻辑外键关系：** user_id -> sys_user.id


### 6.66 `herb_ai_chat_message`：AI 小助手消息表

- **对应模块**：AI 小助手模块
- **优先级/状态**：P1
- **表说明**：保存会话中的用户消息和 AI 回答。
- **通用字段组**：轻量业务表。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| session_id | VARCHAR(64) | 是 | NULL | FK/IDX | 会话 ID |
| user_id | BIGINT | 否 | NULL | FK/IDX | 消息所属用户 ID |
| role | VARCHAR(20) | 是 | NULL |  | 消息角色：user、assistant、system |
| content | LONGTEXT | 是 | NULL |  | 消息内容 |
| model_name | VARCHAR(100) | 否 | NULL |  | AI 模型名称 |
| token_count | INT | 否 | NULL |  | Token 数量，预留 |
| source | VARCHAR(20) | 否 | NULL |  | 来源 |
| error_message | VARCHAR(500) | 否 | NULL |  | AI 调用失败信息 |
...
**普通索引：** idx_ai_chat_message_session_time(user_id, session_id, create_time, id)

**逻辑外键关系：** (user_id, session_id) -> herb_ai_chat_session(user_id, session_id)


### 6.67 `herb_ai_knowledge_doc`：AI 知识库文档表

- **对应模块**：AI 小助手模块 / RAG 知识库模块
- **优先级/状态**：P1
- **表说明**：保存知识库文档原文、类型、启用状态和向量化状态。
- **通用字段组**：轻量业务表。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| doc_code | VARCHAR(64) | 是 | NULL | UK | 文档编码 |
| doc_title | VARCHAR(200) | 是 | NULL |  | 文档标题 |
| doc_type | VARCHAR(30) | 是 | other | IDX | 文档类型：system_guide、herb_knowledge、faq、rule、report、other |
| source_type | VARCHAR(20) | 是 | manual |  | 来源类型：manual、file、markdown、api |
| file_name | VARCHAR(255) | 否 | NULL |  | 文件名 |
| file_url | VARCHAR(500) | 否 | NULL |  | 文件地址 |
| content_text | LONGTEXT | 是 | NULL |  | 文档正文 |
| summary | VARCHAR(1000) | 否 | NULL |  | 文档摘要 |
| status | VARCHAR(20) | 是 | enabled | IDX | 状态：enabled、disabled |
| embedding_status | VARCHAR(20) | 是 | pending | IDX | 向量化状态：pending、processing、completed、failed |
...
**唯一约束 / 唯一索引：** idx_doc_code(doc_code)

**普通索引：** idx_doc_type(doc_type), idx_status(status), idx_embedding_status(embedding_status)

**逻辑外键关系：** 无


### 6.68 `herb_ai_knowledge_chunk`：AI 知识库文档切片表

- **对应模块**：AI 小助手模块 / RAG 知识库模块
- **优先级/状态**：P1
- **表说明**：保存知识库文档切片、内容哈希、向量 ID 和元数据。
- **通用字段组**：轻量业务表。
...
| 字段名 | 数据类型 | 是否必填 | 默认值 | 键/索引 | 字段说明 |
| --- | --- | --- | --- | --- | --- |
| doc_id | BIGINT | 是 | NULL | FK/IDX | 知识库文档 ID |
| chunk_index | INT | 是 | NULL |  | 切片序号 |
| chunk_title | VARCHAR(200) | 否 | NULL |  | 切片标题 |
| chunk_content | LONGTEXT | 是 | NULL |  | 切片内容 |
| content_hash | VARCHAR(64) | 是 | NULL | IDX | 切片内容哈希 |
| token_count | INT | 否 | NULL |  | Token 估算数量 |
| embedding_status | VARCHAR(20) | 是 | pending | IDX | 向量化状态：pending、processing、completed、failed |
| vector_id | VARCHAR(200) | 否 | NULL | IDX | 向量库 ID |
| metadata_json | JSON | 否 | NULL |  | 向量元数据 |
| create_time | DATETIME | 是 | CURRENT_TIMESTAMP |  | 创建时间 |
| update_time | DATETIME | 是 | CURRENT_TIMESTAMP |  | 更新时间 |
| is_deleted | TINYINT | 是 | 0 | IDX | 逻辑删除：0 未删除，1 已删除 |
...
**普通索引：** idx_doc_id(doc_id), idx_content_hash(content_hash), idx_embedding_status(embedding_status), idx_vector_id(vector_id)

**逻辑外键关系：** doc_id -> herb_ai_knowledge_doc.id


---

### 6.69 M12-M15 迁移补充（V20260714_010 至 V20260714_015）

本节与已执行的 Flyway 迁移保持一致，不修改既有表的历史定义。

- `edu_course` 新增 `published_at`、`published_by`；课程资源新增 `sort_order`。
- `rel_project_member` 新增 `member_status`、`left_at`；`research_achievement` 新增 `achievement_stage`、`achievement_status`，并增加 `idx_research_achievement_achievement_status`。
- `edu_experiment_record` 新增 `submitted_at`、`submitted_by`、`archived_by`、`archive_comment`，并增加课程/课题二选一检查约束 `chk_edu_experiment_record_source`。
- `edu_training_plan` 新增 `course_id`、`trainer_id`、`location`、`published_at`、`published_by`，增加 `idx_edu_training_plan_course_id`、`idx_edu_training_plan_trainer_id`。
- `edu_training_record` 新增 `attendance_status`、`checked_in_at`、`result_comment`，增加 `idx_edu_training_record_attendance_status`；`plan_id` 调整为非空。
- `edu_training_record` 增加唯一约束 `uk_edu_training_record_plan_user(plan_id, user_id)`；`edu_training_feedback` 增加唯一约束 `uk_edu_training_feedback_record_user(training_record_id, user_id)`。

新增培训素材表：

- `edu_training_material`：素材编号、名称、类型、文件资源、上传人、复用次数、状态及通用审计字段；唯一约束 `uk_edu_training_material_material_no(material_no)`，索引覆盖名称、类型、文件和上传人。
- `rel_training_plan_material`：培训计划与素材的多对多关联，字段包括 `is_required`、`sort_order`、创建人和备注；唯一约束 `uk_rel_training_plan_material_plan_material(plan_id, material_id)`，并建立计划、素材两个查询索引。

### 6.70 修订记录

| 版本 | 日期 | 修订内容 |
|---|---|---|
| V1.0 | 2026-07-14 | 同步 M12-M15 迁移字段、索引、唯一约束及培训素材表。 |
...
| M09 | 生长采集模块 | herb_growth_record、herb_image、herb_collection_task、herb_batch、herb_batch_image | 已覆盖 |
| M10 | 采集审核与溯源模块 | herb_growth_review_record、herb_growth_record、herb_batch、herb_growth_trace_event | 已覆盖 |
| M11 | 图谱知识模块 | herb_atlas、herb_atlas_tag、herb_atlas_feature、herb_image_feature、herb_image_recognition、herb_image_match、herb_identification_result、herb_batch_image、herb_knowledge_* | 已覆盖 |
...
13. `herb_image_match`
14. `herb_atlas_feature`
15. `herb_image_feature`
16. `herb_identification_result`
17. `herb_collection_task`
18. `herb_batch`
19. `herb_batch_image`
20. `herb_knowledge_triple`
...
| `soil_type` | 生长采集土壤类型 | `sandy`、`loam`、`clay` |
| `weather` | 生长采集天气 | `sunny`、`cloudy`、`rainy` |
| `course_type` | 课程类型 | `experiment`、`training` |
| `course_resource_type` | 课程资源类型 | `document`、`video`、`guide` |
...
图谱知识闭环：标准图谱 → 特征提取 → 用户图片 → 图谱比对/AI 识别 → 最终识别 → 知识实体 → 三元组关系
采集批次闭环：采集任务 → 批次档案 → 图片绑定 → 识别汇总 → 人工复核 → 质量评价与归档
