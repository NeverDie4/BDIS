...
> 本次修订：补充 M05、M06、M19 实现所需字段，并开放加入图像特征、最终识别、采集任务和批次档案 6 张业务表。
...
**普通索引：** idx_sys_file_resource_file_type(file_type), idx_sys_file_resource_uploader_id(uploader_id), idx_sys_file_resource_uploaded_at(uploaded_at), idx_sys_file_resource_access_level(access_level)
...
**普通索引：** idx_log_operation_operator_id(operator_id), idx_log_operation_module(operation_module), idx_log_operation_time(operation_time), idx_log_operation_biz(biz_type, biz_id)
...
| result_status | VARCHAR(50) | 否 | NULL |  | 访问结果：SUCCESS、FAILED |
| ip_address | VARCHAR(64) | 否 | NULL |  | IP 地址 |
| user_agent | VARCHAR(500) | 否 | NULL |  | 客户端信息 |
