...
class TrainingPlanMaterialControllerTest {
    ...
    void setUp() {
        mvc =
                MockMvcBuilders.standaloneSetup(
                                new TrainingPlanMaterialController(service, authorization))
                        .setControllerAdvice(new GlobalExceptionHandler())
                        .build();
    }

    @Test
    void routesAndPermissionsAreCorrect() throws Exception {
        ...
        mvc.perform(
                        post("/training-plans/1/materials")
                                .contentType("application/json")
                                .content("{\"materialId\":2}"))
    ...
    @Test
    void permissionDenialsAndInvalidIdsAreHandled() throws Exception {
        doThrow(new ForbiddenException("denied"))
                .when(authorization)
                .requirePermission("edu:training-material:list");
        mvc.perform(get("/training-plans/1/materials")).andExpect(status().isForbidden());
        reset(authorization);
        doThrow(new ForbiddenException("denied"))
                .when(authorization)
                .requirePermission("edu:training-material:bind");
        mvc.perform(
                        post("/training-plans/1/materials")
                                .contentType("application/json")
                                .content("{\"materialId\":2}"))
