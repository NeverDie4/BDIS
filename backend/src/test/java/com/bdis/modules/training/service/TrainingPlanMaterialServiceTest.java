...
class TrainingPlanMaterialServiceTest {
    ...
    void setUp() {
        service =
                new TrainingPlanMaterialServiceImpl(
                        planMapper,
                        materialMapper,
                        relationMapper,
                        fileMapper,
    ...
    @Test
    void listReturnsJoinedRowsInMapperOrder() {
        TrainingPlanMaterialVO vo = new TrainingPlanMaterialVO();
        vo.setBindingId(1L);
        vo.setSortOrder(10);
    ...
    @Test
    void bindDefaultsAndAtomicallyIncrementsReuseCount() {
        when(planMapper.selectByIdIncludingDeleted(1L)).thenReturn(plan("draft"));
        ...
        when(relationMapper.insert(any(TrainingPlanMaterialEntity.class)))
                .thenAnswer(
                        inv -> {
                            ((TrainingPlanMaterialEntity) inv.getArgument(0)).setId(3L);
                            return 1;
                        });
        when(materialMapper.incrementReuseCount(2L)).thenReturn(1);
        Long id = service.bind(1L, bind(null, null));
        assertEquals(3L, id);
        ArgumentCaptor<TrainingPlanMaterialEntity> c =
                ArgumentCaptor.forClass(TrainingPlanMaterialEntity.class);
    ...
    @Test
    void bindRejectsDuplicateAndConvertsDatabaseUniqueConflict() {
        when(planMapper.selectByIdIncludingDeleted(1L)).thenReturn(plan("draft"));
        when(materialMapper.selectByIdIncludingDeleted(2L)).thenReturn(material(1));
        when(fileMapper.selectById(10L)).thenReturn(file());
        when(relationMapper.selectByPlanAndMaterial(1L, 2L))
                .thenReturn(new TrainingPlanMaterialEntity());
        assertEquals(
    ...
    @Test
    void bindRejectsPublishedClosedMissingOrDisabledMaterial() {
        ...
        assertEquals(
                ...
                assertThrows(BusinessException.class, () -> service.bind(1L, bind(1, 0)))
                        .getResultCode());
        TrainingMaterialEntity deleted = material(1);
    ...
    @Test
    void bindRollsBackWhenCounterUpdateFails() {
        when(planMapper.selectByIdIncludingDeleted(1L)).thenReturn(plan("draft"));
        when(materialMapper.selectByIdIncludingDeleted(2L)).thenReturn(material(1));
        when(fileMapper.selectById(10L)).thenReturn(file());
        when(userMapper.selectById(7L)).thenReturn(user());
    ...
    @Test
    void unbindDeletesOnlyRelationAndAtomicallyDecrements() {
        TrainingPlanMaterialEntity rel = new TrainingPlanMaterialEntity();
        rel.setId(3L);
        rel.setPlanId(1L);
        rel.setMaterialId(2L);
        when(planMapper.selectByIdIncludingDeleted(1L)).thenReturn(plan("draft"));
    ...
    @Test
    void unbindRejectsMissingRelationAndCounterUnderflow() {
        when(planMapper.selectByIdIncludingDeleted(1L)).thenReturn(plan("draft"));
        assertEquals(
                ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class, () -> service.unbind(1L, 2L))
                        .getResultCode());
        TrainingPlanMaterialEntity rel = new TrainingPlanMaterialEntity();
        rel.setId(3L);
        when(relationMapper.selectByPlanAndMaterial(1L, 2L)).thenReturn(rel);
    ...
    @Test
    void hasValidMaterialDelegatesToJoinedValidityQuery() {
    ...
    private TrainingMaterialEntity material(int status) {
        TrainingMaterialEntity m = new TrainingMaterialEntity();
        m.setId(2L);
        m.setFileId(10L);
        m.setStatus(status);
        m.setIsDeleted(0);
    ...
    private TrainingPlanMaterialBindRequest bind(Integer required, Integer sort) {
        TrainingPlanMaterialBindRequest r = new TrainingPlanMaterialBindRequest();
        r.setMaterialId(2L);
        r.setIsRequired(required);
        r.setSortOrder(sort);
        return r;
    }
