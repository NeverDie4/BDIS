...
class TrainingMaterialServiceTest {
    ...
    @Test void pageUsesBatchLookupsAndSupportsFilters() {
        TrainingMaterialEntity entity = material(1L);
        when(materialMapper.selectPage(any(), any())).thenAnswer(inv -> {
            Page<TrainingMaterialEntity> page=inv.getArgument(0); page.setRecords(List.of(entity)); page.setTotal(1); return page;
        });
        ...
        query.setKeyword("MAT"); query.setMaterialType("video"); query.setSourceType("upload");
        query.setUploaderId(7L); query.setStatus(1); query.setSortField("materialName"); query.setSortOrder("asc");
    ...
    @Test void pageRejectsInvalidTypeSourceAndSort() {
        TrainingMaterialQuery q=new TrainingMaterialQuery();
        q.setMaterialType("bad"); assertThrows(BusinessException.class,()->service.page(q));
        q.setMaterialType("video");q.setSourceType("bad");assertThrows(BusinessException.class,()->service.page(q));
        q.setSourceType("upload");q.setSortField("drop table");assertThrows(BusinessException.class,()->service.page(q));
    }

    @Test void detailAggregatesFileUploaderAndPlanCount() {
    ...
    @Test void detailRejectsMissingOrDeleted() {
        assertEquals(ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class,()->service.getDetail(1L)).getResultCode());
        TrainingMaterialEntity deleted=material(1L);deleted.setIsDeleted(1);
        when(materialMapper.selectByIdIncludingDeleted(1L)).thenReturn(deleted);
        assertEquals(ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class,()->service.getDetail(1L)).getResultCode());
    }

    @Test void createDefaultsMetadataAndAudits() {
        when(userMapper.selectById(7L)).thenReturn(user(7L));
        when(fileMapper.selectById(10L)).thenReturn(file(10L,1));
        when(materialMapper.insert(any(TrainingMaterialEntity.class))).thenAnswer(inv->{TrainingMaterialEntity e=inv.getArgument(0);e.setId(11L);return 1;});
        assertEquals(11L,service.create(create("upload",null)));
        ArgumentCaptor<TrainingMaterialEntity> c=ArgumentCaptor.forClass(TrainingMaterialEntity.class);
    ...
    @Test void createEnforcesPermanentMaterialNumber() {
        when(userMapper.selectById(7L)).thenReturn(user(7L));
        TrainingMaterialEntity deleted=material(99L);deleted.setIsDeleted(1);
        when(materialMapper.selectByMaterialNoIncludingDeleted("MAT-1")).thenReturn(deleted);
        assertEquals(ResultCodeEnum.CONFLICT,
                assertThrows(BusinessException.class,()->service.create(create("upload",null))).getResultCode());
        verify(fileMapper,never()).selectById(anyLong());
    }

    @Test void createRejectsInvalidTypeSourceAndFile() {
        TrainingMaterialCreateRequest r=create("upload",null);r.setMaterialType("bad");
        assertThrows(BusinessException.class,()->service.create(r));
        r.setMaterialType("video");r.setSourceType("bad");
        assertThrows(BusinessException.class,()->service.create(r));
        r.setSourceType("upload");
        when(userMapper.selectById(7L)).thenReturn(user(7L));
        assertEquals(ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class,()->service.create(r)).getResultCode());
        FileResourceEntity deleted=file(10L,1);deleted.setIsDeleted(1);
        when(fileMapper.selectById(10L)).thenReturn(deleted);
        assertEquals(ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class,()->service.create(r)).getResultCode());
        FileResourceEntity inactive=file(10L,0);when(fileMapper.selectById(10L)).thenReturn(inactive);
        assertEquals(ResultCodeEnum.NOT_FOUND,
                assertThrows(BusinessException.class,()->service.create(r)).getResultCode());
    }

    @Test void courseResourceSourceRequiresMatchingActiveResourceAndFile() {
        when(userMapper.selectById(7L)).thenReturn(user(7L));
        when(fileMapper.selectById(10L)).thenReturn(file(10L,1));
        CourseResourceEntity resource=new CourseResourceEntity();
        resource.setId(20L);resource.setFileId(10L);resource.setStatus(1);resource.setIsDeleted(0);
        when(courseResourceMapper.selectById(20L)).thenReturn(resource);
        when(materialMapper.insert(any(TrainingMaterialEntity.class))).thenAnswer(inv->{((TrainingMaterialEntity)inv.getArgument(0)).setId(1L);return 1;});
    ...
    @Test void uploadAndGeneratedRejectSourceResourceButCourseResourceRequiresIt() {
    ...
    @Test void updateUsesVersionAllowsDisableAndRejectsImmutableNumber() {
        TrainingMaterialEntity entity=material(1L);
        ...
        TrainingMaterialUpdateRequest r=update();r.setStatus(0);
        service.update(1L,r);
    ...
    @Test void updateRejectsVersionConflictAndZeroAffectedRows() {
        TrainingMaterialEntity entity=material(1L);
        when(materialMapper.selectByIdIncludingDeleted(1L)).thenReturn(entity);
        TrainingMaterialUpdateRequest r=update();r.setVersion(9);
        assertEquals(ResultCodeEnum.CONFLICT,
    ...
    @Test void deleteUnreferencedUsesLogicalDeleteAndNeverDeletesFile() {
        TrainingMaterialEntity entity=material(1L);
        when(materialMapper.selectByIdIncludingDeleted(1L)).thenReturn(entity);
    ...
    @Test void deleteReferencedMaterialReturnsConflict() {
        when(materialMapper.selectByIdIncludingDeleted(1L)).thenReturn(material(1L));
    ...
    @Test void auditFailurePropagates() {
        when(userMapper.selectById(7L)).thenReturn(user(7L));
        when(fileMapper.selectById(10L)).thenReturn(file(10L,1));
        when(materialMapper.insert(any(TrainingMaterialEntity.class))).thenAnswer(inv->{((TrainingMaterialEntity)inv.getArgument(0)).setId(1L);return 1;});
    ...
    private TrainingMaterialCreateRequest create(String source,Long sourceId){
        ...
        r.setMaterialNo("MAT-1");r.setMaterialName("Material");r.setMaterialType("video");
        r.setFileId(10L);r.setSourceType(source);r.setSourceResourceId(sourceId);return r;
    }
    private TrainingMaterialUpdateRequest update(){
        TrainingMaterialUpdateRequest r=new TrainingMaterialUpdateRequest();
        r.setMaterialName("Updated");r.setMaterialType("video");r.setFileId(10L);
        r.setSourceType("upload");r.setStatus(1);r.setVersion(0);return r;
    }
    private TrainingMaterialEntity material(Long id){
        TrainingMaterialEntity e=new TrainingMaterialEntity();e.setId(id);e.setMaterialNo("MAT-1");
        e.setMaterialName("Material");e.setMaterialType("video");e.setFileId(10L);e.setSourceType("upload");
        e.setUploaderId(7L);e.setReuseCount(0);e.setStatus(1);e.setIsDeleted(0);e.setVersion(0);return e;
    }
    private FileResourceEntity file(Long id,int status){
        FileResourceEntity f=new FileResourceEntity();f.setId(id);f.setFileName("file.pdf");
        f.setOriginalFilename("original.pdf");f.setStatus(status);f.setIsDeleted(0);return f;
    }
    private UserEntity user(Long id){
        UserEntity u=new UserEntity();u.setId(id);u.setRealName("User "+id);u.setStatus(1);u.setIsDeleted(0);return u;
    }
}
