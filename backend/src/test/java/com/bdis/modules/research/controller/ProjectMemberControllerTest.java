...
class ProjectMemberControllerTest {
    ...
    void setUp() {
        mockMvc =
                MockMvcBuilders.standaloneSetup(
                                new ProjectMemberController(memberService, authorizationService))
    ...
    void addUpdateAndRemoveUseDedicatedPermissions() throws Exception {
        ...
        vo.setId(20L);
        when(memberService.add(
                        org.mockito.ArgumentMatchers.eq(10L), any(ProjectMemberAddRequest.class)))
                .thenReturn(20L);
        when(memberService.get(10L, 7L)).thenReturn(vo);

        mockMvc.perform(
                        post("/research-projects/10/members")
                                .contentType("application/json")
                                .content("{\"userId\":7,\"memberRole\":\"student\"}"))
                .andExpect(status().isOk());
        mockMvc.perform(
                        put("/research-projects/10/members/7")
                                .contentType("application/json")
                                .content("{\"memberRole\":\"researcher\"}"))
                .andExpect(status().isOk());
        mockMvc.perform(delete("/research-projects/10/members/7")).andExpect(status().isOk());
