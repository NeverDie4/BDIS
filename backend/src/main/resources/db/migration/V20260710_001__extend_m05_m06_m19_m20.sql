...
ALTER TABLE `log_file_access`
  ...
  ADD COLUMN `result_status` VARCHAR(50) NULL DEFAULT NULL COMMENT '访问结果' AFTER `access_type`;
...
ALTER TABLE `log_data_sync`
  ...
  ADD COLUMN `biz_type` VARCHAR(100) NULL DEFAULT NULL COMMENT '导入业务对象类型' AFTER `exchange_id`,
  ADD COLUMN `biz_id` BIGINT NULL DEFAULT NULL COMMENT '导入业务对象 ID' AFTER `biz_type`,
  ADD COLUMN `external_no` VARCHAR(100) NULL DEFAULT NULL COMMENT '外部系统业务编号' AFTER `biz_id`,
...
ALTER TABLE `soap_sync_task`
  ...
  ADD COLUMN `is_mock` TINYINT NOT NULL DEFAULT 1 COMMENT '是否使用模拟 SOAP 响应' AFTER `sync_status`;
