...
public class CourseLearningProgressServiceImpl implements CourseLearningProgressService {
    ...
    public CourseLearningProgressServiceImpl(
    ...
    public CourseLearningProgressVO save(Long courseId, CourseLearningProgressRequest request) {
        ...
        CourseLearningProgressEntity entity =
                progressMapper.selectActive(
                        enrollment.getId(), request.getItemType(), request.getItemId());
        if (entity == null) {
            ...
            entity.setFirstAccessedAt(now);
            entity.setStatus(1);
            entity.setIsDeleted(0);
            entity.setCreatedAt(now);
            entity.setCreatedBy(enrollment.getUserId());
            entity.setVersion(0);
        }
        entity.setProgressValue(
                request.getProgressValue() == null
                        ? (request.getCompleted() ? BigDecimal.valueOf(100) : BigDecimal.ZERO)
                        : request.getProgressValue());
        entity.setProgressSeconds(request.getProgressSeconds());
        ...
        return progressMapper.selectVOByEnrollment(enrollment.getId()).stream()
                .filter(
                        item ->
                                item.getItemType().equals(request.getItemType())
                                        && item.getItemId().equals(request.getItemId()))
                .findFirst()
                .orElseThrow(() -> new BusinessException("Learning progress was not saved"));
    }
    ...
    public CourseLearningSummaryVO summary(Long courseId) {
        ...
        CourseLearningSummaryVO vo = new CourseLearningSummaryVO();
        ...
        vo.setAverageScore(
                scores.stream()
                        ...
                        .divide(
                                BigDecimal.valueOf(Math.max(1, scores.size())),
                                2,
                                RoundingMode.HALF_UP));
        vo.setExcellentCount(
                (int)
                        scores.stream()
                                ...
                                .filter(
                                        s ->
                                                s.compareTo(BigDecimal.valueOf(60)) >= 0
                                                        && s.compareTo(BigDecimal.valueOf(90)) < 0)
                                .count());
        vo.setFailCount(
                (int) scores.stream().filter(s -> s.compareTo(BigDecimal.valueOf(60)) < 0).count());
        return vo;
    }
    ...
    private CourseEnrollmentEntity requireEnrollment(Long courseId) {
        ...
        CourseEnrollmentEntity enrollment =
                enrollmentMapper.selectActiveByCourseAndUser(courseId, userId);
        if (enrollment == null) {
            throw new ForbiddenException("Please enroll in the course first");
        }
        return enrollment;
    }

    private void validateItem(
            Long courseId, CourseLearningProgressRequest request, Long enrollmentId) {
        if (!"step".equals(request.getItemType())
                && !"resource".equals(request.getItemType())
                && !"video".equals(request.getItemType())) {
            throw new BusinessException("Unsupported learning item type");
        }
        if ("step".equals(request.getItemType())
                && progressMapper.existsActiveStep(courseId, request.getItemId()) == 0) {
            throw new BusinessException("Experiment step does not belong to this course");
        }
        if ("step".equals(request.getItemType())
                && progressMapper.countIncompletePreviousSteps(
                                courseId, request.getItemId(), enrollmentId)
                        > 0) {
            throw new BusinessException("Experiment steps must be completed in order");
        }
        if ("resource".equals(request.getItemType())
                && progressMapper.existsActiveResource(courseId, request.getItemId()) == 0) {
            throw new BusinessException("Course resource does not belong to this course");
        }
        if ("video".equals(request.getItemType())
                && progressMapper.existsActiveVideoResource(courseId, request.getItemId()) == 0) {
            throw new BusinessException("Video resource does not belong to this course");
        }
    }

    private void refreshEnrollment(CourseEnrollmentEntity enrollment, LocalDateTime now) {
        int total =
                progressMapper.countActiveSteps(enrollment.getCourseId())
                        + progressMapper.countActiveResources(enrollment.getCourseId());
        int completed =
                progressMapper.countCompletedValidItems(
        ...
        enrollment.setEnrollmentStatus(
                progress.compareTo(BigDecimal.valueOf(100)) >= 0 ? "completed" : "enrolled");
        enrollment.setCompletedAt(progress.compareTo(BigDecimal.valueOf(100)) >= 0 ? now : null);
        enrollment.setLastAccessedAt(now);
        enrollment.setUpdatedAt(now);
        enrollment.setUpdatedBy(enrollment.getUserId());
