...
public interface CourseEnrollmentMapper extends BaseMapper<CourseEnrollmentEntity> {
    ...
    @Select("SELECT * FROM edu_course_enrollment WHERE course_id = #{courseId} AND user_id = #{userId} AND is_deleted = 0 LIMIT 1")
    CourseEnrollmentEntity selectActiveByCourseAndUser(@Param("courseId") Long courseId, @Param("userId") Long userId);

    @Select("SELECT ce.id, ce.course_id, ce.user_id, ce.enrollment_status, ce.progress, ce.score, ce.enrolled_at, ce.completed_at, ce.last_accessed_at, u.username, u.real_name AS user_name FROM edu_course_enrollment ce LEFT JOIN sys_user u ON u.id = ce.user_id WHERE ce.course_id = #{courseId} AND ce.is_deleted = 0 ORDER BY ce.enrolled_at DESC")
    ...
    @Select("SELECT COUNT(1) FROM edu_course_enrollment WHERE course_id=#{courseId} AND user_id=#{userId} AND enrollment_status='completed' AND is_deleted=0")
