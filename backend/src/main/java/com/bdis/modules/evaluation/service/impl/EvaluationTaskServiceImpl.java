...
public class EvaluationTaskServiceImpl implements EvaluationTaskService {
    ...
    public IPage<EvaluationTaskEntity> listTasks(EvaluationTaskQuery query) {
        ...
        LambdaQueryWrapper<EvaluationTaskEntity> wrapper =
                new LambdaQueryWrapper<EvaluationTaskEntity>()
                        .like(StringUtils.hasText(safeQuery.getKeyword()), EvaluationTaskEntity::getTaskName, safeQuery.getKeyword())
                        .eq(StringUtils.hasText(safeQuery.getTargetType()), EvaluationTaskEntity::getTargetType, safeQuery.getTargetType())
                        .eq(safeQuery.getTargetId() != null, EvaluationTaskEntity::getTargetId, safeQuery.getTargetId())
                        .eq(StringUtils.hasText(safeQuery.getStatus()), EvaluationTaskEntity::getTaskStatus, safeQuery.getStatus())
                        .orderByDesc(EvaluationTaskEntity::getUpdatedAt);
        return taskMapper.selectPage(page(safeQuery.getPageNum(), safeQuery.getPageSize()), wrapper);
    }
    ...
    public EvaluationTaskEntity createTask(EvaluationTaskRequest request) {
        ...
        entity.setTaskStatus(defaultText(request.getTaskStatus(), "draft"));
    ...
    public EvaluationTaskDetailVO getTaskDetail(Long taskId) {
        ...
        EvaluationResultEntity result =
                resultMapper.selectOne(new LambdaQueryWrapper<EvaluationResultEntity>().eq(EvaluationResultEntity::getTaskId, taskId));
