...
public class EvaluationStandardServiceImpl implements EvaluationStandardService {
    ...
    public IPage<EvaluationIndicatorEntity> listStandards(EvaluationStandardQuery query) {
        ...
        LambdaQueryWrapper<EvaluationIndicatorEntity> wrapper =
                new LambdaQueryWrapper<EvaluationIndicatorEntity>()
                        .like(StringUtils.hasText(safeQuery.getKeyword()), EvaluationIndicatorEntity::getIndicatorName, safeQuery.getKeyword())
                        .eq(StringUtils.hasText(safeQuery.getIndicatorType()), EvaluationIndicatorEntity::getIndicatorType, safeQuery.getIndicatorType())
                        .eq(safeQuery.getStatus() != null, EvaluationIndicatorEntity::getStatus, safeQuery.getStatus())
                        .orderByAsc(EvaluationIndicatorEntity::getSortOrder)
                        .orderByDesc(EvaluationIndicatorEntity::getUpdatedAt);
        return indicatorMapper.selectPage(page(safeQuery.getPageNum(), safeQuery.getPageSize()), wrapper);
    }
    ...
    @Override
    public EvaluationIndicatorEntity updateStandard(Long standardId, EvaluationStandardRequest request) {
    ...
    private void fillEntity(EvaluationIndicatorEntity entity, EvaluationStandardRequest request) {
        ...
        entity.setMaxScore(request.getMaxScore() == null ? new BigDecimal("100.00") : request.getMaxScore());
