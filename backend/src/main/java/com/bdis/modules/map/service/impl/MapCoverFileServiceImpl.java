...
public class MapCoverFileServiceImpl implements MapCoverFileService {
    ...
    @Override
    @Transactional
    public String replaceCover(Long pointId, String previousFileUrl, String requestedFileUrl) {
        Long previousFileId =
        ...
        if (!StringUtils.hasText(requestedFileUrl)) {
            releaseIfBound(pointId, previousFileId);
            return null;
        }
        if (requestedFileId == null) {
            throw new BusinessException(ResultCodeEnum.VALIDATION_ERROR, "地图封面必须使用文件上传模块返回的图片");
        }

        String publicUrl = publishAndBind(pointId, requestedFileId);
    ...
    @Override
    @Transactional
    public void deleteCover(Long pointId, String fileUrl) {
        Long fileId =
                StringUtils.hasText(fileUrl) ? fileResourceService.resolveFileId(fileUrl) : null;
        releaseIfBound(pointId, fileId);
    }

    private String publishAndBind(Long pointId, Long fileId) {
    ...
    private void releaseIfBound(Long pointId, Long fileId) {
        if (fileId == null) {
            return;
        }
        long mapBindingCount =
                fileBusinessMapper.selectCount(
                        new LambdaQueryWrapper<FileBusinessEntity>()
                                ...
                                .eq(FileBusinessEntity::getBizId, pointId)
                                .eq(FileBusinessEntity::getFileUsage, FILE_USAGE));
        if (mapBindingCount == 0) {
            return;
        }

        fileBusinessService.deleteByBusinessAndFile(BIZ_TYPE, pointId, fileId);
