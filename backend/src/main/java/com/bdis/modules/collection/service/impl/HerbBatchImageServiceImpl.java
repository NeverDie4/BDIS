...
public class HerbBatchImageServiceImpl implements HerbBatchImageService {
  ...
  public HerbBatchImageVO bind(Long batchId, HerbBatchImageBindRequest request) {
    ...
    publishImageEvent(batch, entity.getImageId());
  ...
  public HerbBatchImageBindResultVO batchBind(
    ...
    refreshImageCount(batch);
    entities.forEach(entity -> publishImageEvent(batch, entity.getImageId()));
  ...
  private void publishImageEvent(HerbBatchEntity batch, Long imageId) {
    if (agentEventPublisher != null) {
      agentEventPublisher.publish(
          AgentFieldDataChangedEvent.Type.IMAGE_UPLOADED,
          batch.getTaskId(),
          batch.getId(),
          imageId,
          null);
  ...
  @Override
  public List<HerbBatchImageVO> listByTask(Long taskId) {
    if (taskId == null) {
    ...
    List<Long> batchIds =
        herbBatchMapper.selectByTaskId(taskId).stream()
            .map(HerbBatchVO::getId)
            .filter(id -> id != null)
            .toList();
    return batchIds.isEmpty()
        ? List.of()
        : herbBatchImageMapper.selectBatchImagesWithIdentificationByBatchIds(batchIds);
  }
  ...
  @Override
  @Transactional
  public HerbBatchImageVO update(Long batchId, Long imageId, HerbBatchImageUpdateRequest request) {
    HerbBatchEntity batch = getEditableBatch(batchId);
    HerbBatchImageEntity binding = getBoundBinding(batch.getId(), imageId);
    if (request == null) {
  ...
  private HerbBatchImageEntity buildBinding(
    ...
    if (checkDuplicate
        && herbBatchImageMapper.selectByBatchIdAndImageId(batchId, request.getImageId()) != null) {
