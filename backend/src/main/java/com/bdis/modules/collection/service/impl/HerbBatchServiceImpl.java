...
public class HerbBatchServiceImpl implements HerbBatchService {
  ...
  public HerbBatchVO create(HerbBatchCreateRequest request) {
    ...
    publishAgentEvent(AgentFieldDataChangedEvent.Type.BATCH_CREATED, entity);
    return entity.getId() == null ? toVO(entity) : getById(entity.getId());
  }

  private void publishAgentEvent(AgentFieldDataChangedEvent.Type type, HerbBatchEntity batch) {
    if (agentEventPublisher != null) {
      agentEventPublisher.publish(type, batch.getTaskId(), batch.getId(), null, null);
    }
  }
