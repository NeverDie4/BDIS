...
public interface TrainingWorkflowService {
    TrainingReportVersionEntity submit(Long id, TrainingReportSubmitRequest request);

    void review(Long id, TrainingRecordReviewRequest request);

    List<TrainingEvaluationEntity> evaluations(Long id);

    void evaluate(Long id, TrainingEvaluationRequest request);

    TrainingCompletionProofVO proof(Long id);

    List<TrainingReportVersionEntity> reports(Long id);
}
