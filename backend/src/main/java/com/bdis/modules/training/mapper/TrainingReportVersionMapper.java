...
public interface TrainingReportVersionMapper extends BaseMapper<TrainingReportVersionEntity> {
    @Select(
            "SELECT * FROM edu_training_record_report_version WHERE training_record_id=#{recordId} ORDER BY version_no DESC")
    List<TrainingReportVersionEntity> selectByRecordId(@Param("recordId") Long recordId);
}
