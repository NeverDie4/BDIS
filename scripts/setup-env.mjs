...
for (const item of templates) {
  ...
  console.log(`[done] created ${displayPath(item.target)} from ${displayPath(item.source)}`);
...
console.log('\nNext steps:');
console.log('1. Edit .env and frontend/.env.local for your local database, Redis, and API URL.');
