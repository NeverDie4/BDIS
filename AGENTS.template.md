...
- 当前阶段：2026-07-16 冻结基线。只处理阻断性缺陷、安全问题、配置问题和文档勘误；新增范围须经小组确认。
...
8. 冻结边界：不得趁修复任务新增业务模块、扩大角色权限、改变状态机或做无关重构；确需调整时先说明影响并获得确认。
9. 迁移不可变：不得修改、重命名或删除已发布的 Flyway migration；历史脚本内容和版本必须通过基线 SHA-256 校验。
...
- 冻结范围与文档优先级：[docs/09_冻结版功能范围与文档索引_V1.0_全组_20260716.md](docs/09_冻结版功能范围与文档索引_V1.0_全组_20260716.md)
- 技术栈：[docs/03_技术栈详细设计规范_V1.0_全组_20260707.md](docs/03_技术栈详细设计规范_V1.0_全组_20260707.md)
- 接口清单：[docs/04_接口设计规范与接口清单_V1.2_全组_20260716.md](docs/04_接口设计规范与接口清单_V1.2_全组_20260716.md)
...
- 脚本命令：[docs/10_脚本命令使用指南_V1.2_全组_20260716.md](docs/10_脚本命令使用指南_V1.2_全组_20260716.md)
- 数据库迁移：[docs/10_Flyway数据库迁移规则_V1.0_全组_20260708.md](docs/10_Flyway数据库迁移规则_V1.0_全组_20260708.md)
...
后端 Maven 命令必须通过 `node scripts/run-maven.mjs` 或根目录 `pnpm` 脚本执行，以确保使用 Java 21。代码变更按风险运行相关测试，提交前优先运行 `pnpm validate`；只修改文档时可不运行完整构建，但必须说明原因。
...
- 未经确认不得扩大需求范围或引入无关重构。
- 不得把 Agent 过程计划、缓存、个人 Apifox 环境、构建产物或本地数据库快照作为交付物提交。
...
- Flyway 迁移脚本必须和数据库设计文档保持一致；新增脚本只能追加大于 `dev` 基线最高版本的全局唯一版本号。
- `backend/src/test/resources/flyway-dev-migration-baseline.txt` 中已发布 migration 的文件名与 SHA-256 不得改写；不得以 `repair`、手工修改历史表或重命名旧脚本掩盖校验失败。
...
- 文档内容与代码、OpenAPI、迁移不一致时，按冻结版文档索引确定事实来源，并同步修正文档或归档过时草稿。
